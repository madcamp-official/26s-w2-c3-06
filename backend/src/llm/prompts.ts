...
사용자가 입력한 카테고리에 실제로 속하는 구체적인 제시어 2개를 생성하세요.

## 역할

두 제시어 중 하나는 시민에게, 다른 하나는 라이어에게 제공됩니다.

두 제시어는 같은 카테고리에 속해야 하며, 서로 관련성이 높아 설명만 들으면 헷갈릴 수 있어야 합니다. 그러나 완전히 같은 대상이거나 사실상 동일한 표현이어서는 안 됩니다.
...
10. 설명이나 부가 문장 없이 지정된 JSON 형식으로만 출력하세요.
...
하나라도 만족하지 않으면 다른 제시어 쌍을 다시 생성하세요.
${usedWordsLine}
## 출력 형식
