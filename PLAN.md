...
**게임 규칙**: 방장이 카테고리(직접 입력 또는 "AI 랜덤 생성")와 AI 봇 수를 정해 게임을 시작한다. 참가자(사람+봇) 중 라이어(MVP에서는 1명 고정, 추후 방장이 수 선택 가능)는 **자신이 라이어인지 모른 채** 진짜 제시어와 비슷하지만 다른 가짜 제시어를 받는다. 각자 제시어를 직접 말하지 않고 설명한 뒤 익명 투표로 라이어를 지목하고, 지목된 사람이 실제 라이어면 진짜 제시어를 맞혀 역전승할 기회를 준다.

AI는 세 지점에 개입해 "LLM Wrapper" 요소를 드러낸다:
1. 방장이 지정(또는 AI 랜덤)한 카테고리로 진짜/가짜 제시어 쌍을 생성
2. 방장이 선택한 수만큼 AI 봇이 플레이어로 참여해 설명 생성 (봇도 자신이 라이어인지 모름 — 사람과 동일 조건)
3. **매 턴** 설명이 제출될 때마다 AI가 일부러 헷갈리게 하는 "교란" 코멘트를 단다
...
- **LLM: Anthropic Claude API (추천)**
  - 세 함수(제시어 쌍 생성, 봇 턴 생성, 매 턴 교란 코멘트) 모두 빈도가 높거나 지연에 민감 → **Claude Haiku 4.5**로 시작.
...
3. **로비** — 공개방/비공개방 진입, 로그아웃 버튼
4. **방 페이지(게임 진행)** — 로그인/로그아웃 버튼 없음 (의도적 설계, 아래 "계정 전환 충돌 회피" 참고)
...
interface GameState {
  ...
  realWord: string;
  liarWord: string;
  liarIds: string[];            // 서버 전용 비밀, 라이어 본인에게도 전송 안 함. MVP: 길이 1 고정. 추후 방장이 라이어 수 선택 시 길이 증가
...
- `generateTurnComment` 프롬프트 핵심: 방금 제출된 설명을 보고 **의도적으로 헷갈리게 만드는** 코멘트를 생성. 실제 라이어가 누구인지는 절대 이 프롬프트에 입력하지 않음 — 정답을 아는 채로 교란하면 너무 정교해져 게임이 망가지므로, 봇과 같은 원칙("정답을 모르는 관전자"처럼 행동)을 따라야 자연스러운 노이즈가 된다.
...
- **미구현으로 남은 것**: 아래 "MVP 제외(stretch)"·"TODO" 항목(라운드 재시작, `room:rejoin` 재접속, 라이어 수 선택, 준비(isReady) 기능 정렬, 방별 네임스페이스)
...
- **`RoomSummary` 필드 정리**: 계약상 `room:publicList`는 `{ roomCode, playerCount }`만 주고 `room:create`에 방 제목이 없으며 인원 상·하한도 두지 않는다. 프론트의 `title`·`maxPlayers`는 백엔드 소스가 없으므로 제거한다(또는 표시하지 않는다).

## MVP 제외 (stretch)

- 게임 내 라운드 재시작(여러 라운드 반복)
- ~~로컬 DB 연결 및 유저 전적(전체 게임수·전체/라이어/비라이어 승률) 저장~~ — **백엔드 구현 완료** (스키마는 "DB 스키마" 섹션 참조)
...
- **라이어 수 선택**: MVP는 라이어 1명 고정이지만, 데이터 모델을 `liarIds: string[]`(길이 1)로 두어 확장 여지를 남겼다. 추후 방장이 게임 설정에서 라이어 수를 고를 수 있게 하려면 `game:configure`에 `liarCount`를 추가하고, 라이어 배정·투표 판정(라이어 전원 색출 여부 등 승패 규칙)을 다인 라이어 기준으로 정의한다.
- **재접속(`room:rejoin`) 지원**: 모바일 환경에서 네트워크 끊김이 흔하므로 최소한의 재접속 처리가 필요할지 나중에 검토한다. 지금 MVP에는 넣지 않는다.
- **준비(isReady) 기능 유지 여부**: 프론트 골격은 "모두 준비 완료 시 방장이 게임 시작" 방식(`Player.isReady`, 준비 토글)을 이미 구현했지만, 이 문서의 계약에는 준비 개념이 없고 방장이 `game:configure`로 바로 시작한다. **어느 쪽으로 정렬할지는 프론트 팀원과 상의 후 결정**하고, 결정되면 이벤트 계약·데이터 모델(`player:ready` 이벤트, `Player.isReady`)을 그에 맞게 반영한다.
