...
**게임 규칙**: 방장이 카테고리(직접 입력 또는 "AI 랜덤 생성")와 AI 봇 수를 정해 게임을 시작한다. 참가자(사람+봇) 중 한 명(라이어)은 **자신이 라이어인지 모른 채** 진짜 제시어와 비슷하지만 다른 가짜 제시어를 받는다. 각자 제시어를 직접 말하지 않고 설명한 뒤 익명 투표로 라이어를 지목하고, 지목된 사람이 실제 라이어면 진짜 제시어를 맞혀 역전승할 기회를 준다.
