...
- 모든 필수 설명을 제출한 플레이어에게만 정상 경험치(위 표의 `baseExp`)를 지급한다.
- 투표나 설명을 완료하지 않은 경우 획득 경험치의 50%만 지급한다.
...
- `participationMultiplier`: 설명 제출 + 투표 완료 시 1.0, 투표 미완료 시 0.5, 설명 미제출(및 투표도 안 함) 또는 고의 이탈 시 0.
...
- 모든 플레이어가 최소 한 번 이상 설명을 제출해야 정상 게임으로 인정한다.

> **구현 참고(코드 미반영)**: 위 정책을 실제로 계산하려면 `GamePlay`에 현재 없는 필드(플레이어별 설명 제출 여부, 투표 완료 여부, 투표 대상이 라이어였는지 등)가 추가로 필요하다. 이번 문서 갱신에서는 정책만 정의하고 구현(`schema.prisma`/`gamePlayRepo.ts`/`gameEngine.ts`)은 별도 작업으로 남겨둔다.
...
- `turn:skip` `{}` — 호스트 전용. 설명 페이즈에서 현재 턴(사람이든 생각 중인 봇이든)을 시간 초과와 동일하게 처리해 강제로 다음 사람에게 넘긴다. 설명 페이즈가 아니거나 호스트가 아니면 무시
...
- **`turn:skip`**: room_screen.dart 설명 페이즈에 방장 전용 "다음 차례 ▶" 버튼을 두고, 누르면 `socket_service.dart`의 `skipTurn()`으로 emit한다.
