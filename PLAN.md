...
각 화면·페이즈에서 실제로 어떻게 동작해야 하는지를 정의하는 세 축 — 게임 자체의 규칙, 그 결과로 쌓이는 경험치/레벨 시스템, 그리고 게임에 들어오기 전 계정·로그인 흐름 — 을 하나로 묶는다.
...
**경험치(EXP) 및 레벨**: 누적 EXP는 `User` 테이블의 별도 컬럼(`exp` 정수형, 기본값 0, **단조증가만 가능 — 절대 감소하지 않는다**)으로 저장한다. 레벨은 누적 EXP에서 계산하는 파생값이며, DB나 API에 직접 저장되지 않는다(매번 필요할 때 계산). 정책 상세는 위 [경험치(EXP) 및 레벨 정책](#경험치exp-및-레벨-정책) 참고.
...
16. `dev` 클린 체크아웃에서 `backend`/`frontend` 두 디렉터리가 충돌 없이 공존하며 각자 정상 실행되는지 확인
...
- **Storage CORS origin 좁히기 — ✅ 완료**: Firebase Storage 버킷(`l-ai-r-game.firebasestorage.app`)의 CORS를 배포 도메인 2개 + 로컬 개발용 `http://localhost:3000`으로 좁혔다(`gsutil cors set`, 설정 파일은 `backend/firebase/storage-cors.json`에 참고용으로 보관 — storage.rules와 마찬가지로 자동 배포되지 않으니 수정 시 다시 수동으로 `gsutil cors set` 실행 필요). **단, 중요한 한계가 있다**: 이 버킷 CORS는 원본 GCS API(`storage.googleapis.com`)에는 적용되지만, Flutter 앱이 실제로 쓰는 Firebase SDK 엔드포인트(`firebasestorage.googleapis.com`)는 버킷 CORS 설정을 무시하고 항상 `Access-Control-Allow-Origin: *`를 반환한다(Firebase 자체의 알려진 동작, 프로젝트 설정과 무관). 즉 이 작업은 SDK 경유 업로드/다운로드 경로의 실질적 보안 강화 효과는 없고, 진짜 방어선은 여전히 아래 Storage Rules(로그인 + 본인 uid 확인)뿐이다.
...
**빌더 강제(`railway.json`)**: Railway는 기본적으로 자체 빌드팩(Railpack)으로 저장소를 자동 감지하는데, 리포 루트에 `backend/`·`frontend/` 서브폴더만 있는 구조라 Railpack이 "어떻게 빌드할지 모르겠다"며 실패한다. 리포 루트의 `railway.json`(`"build": { "builder": "DOCKERFILE", "dockerfilePath": "Dockerfile" }`)으로 Railpack 대신 반드시 이 Dockerfile을 쓰도록 못박아둔다. Railway 서비스 설정의 Root Directory는 비워둔 채(=리포 루트) 사용한다.

**웹에서 백엔드 주소 결정**: 프론트가 백엔드와 same-origin으로 서빙되므로, `frontend/lib/api/backend_config.dart`는 웹에서 `--dart-define=BACKEND_URL`이 명시되지 않으면 빌드 시점 상수 대신 런타임에 `Uri.base.origin`(현재 페이지 주소)을 그대로 쓴다 — 그래야 Railway 기본 도메인이든 커스텀 도메인이든 재빌드 없이 항상 자기 자신을 호출한다. 비웹(안드로이드 에뮬레이터 등 로컬 개발)은 기존처럼 `localhost:3000` 기본값을 쓴다.

### 추적 브랜치(Deployment Tracking)
이 통합 배포이므로 `backend`와 `frontend` 코드가 한 브랜치에 함께 있어야 한다.
- **개발 중(`dev`)**: 백엔드/프론트 변경이 커밋으로 바로 `dev`에 반영되므로, Railway가 `dev`를 자동 추적하는 게 기본. 최신 코드가 지속적으로 배포되는 개발 환경(staging 역할).
- **현재 상태(`main`)**: 2026-07-14부로 Railway 추적 브랜치를 `dev`에서 `main`으로 이미 전환했다(제출/데모 대비 조기 전환). `dev` → `main`은 CLAUDE.md "Merge Policy"대로 PR 없이 직접 `git merge`로 반영하며, push 즉시 Railway가 자동 재배포한다.

### 커스텀 도메인
캠프에서 제공하는 DNS 셀프서비스(`dns.madcamp-kaist.org`)로 `l-ai-r-game.madcamp-kaist.org`를 추가 서브도메인으로 신청하고, Railway가 요구하는 CNAME(`@` → Railway가 준 타깃 호스트)과 TXT(`_railway-verify.l-ai-r-game` → 소유권 검증 값) 레코드를 등록해 Railway Networking의 Custom Domain으로 연결했다. TXT 레코드는 이름이 언더스코어로 시작해 셀프서비스 API가 거부해서(`INVALID_RECORD_NAME`), 캠프 기술지원(박지민)에게 직접 추가를 요청해 처리했다. Railway 기본 도메인(`*.up.railway.app`)도 커스텀 도메인 DNS 장애 시 백업 접속 경로로 계속 유지한다.

Google 로그인이 이 두 도메인 모두에서 동작하도록 Firebase 콘솔 → Authentication → Settings → **Authorized domains**에 `l-ai-r-game.madcamp-kaist.org`와 `l-ai-r-game.up.railway.app`을 등록해뒀다(미등록 도메인에서는 `auth/unauthorized-domain`으로 실패).
