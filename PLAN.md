...
- **XP·레벨 프론트 표시**: 백엔드가 `GET /api/users/me`로 누적 `xp`(저장값)와 계산된 `level`(파생값)을 내려주고, 로비 전적 카드에서 `Lv.{level} ({xp} XP)` 형태로 표시한다(`lobby_screen.dart`). 프로필 화면에도 레벨 배지와 레벨 내 진행바(`_LevelBadge`)를 표시한다 — 진행도 계산은 `현재 레벨 시작점(누적 XP) = 100×(level−1) + 15×(level−1)×(level−2)`, `다음 레벨까지 필요 증분 XP = 100 + (level−1)×30`, `진행도 = (xp − 현재_레벨_시작점) / 다음_레벨까지_필요_증분`으로 계산(`UserStats.levelProgress`/`xpToNextLevel`).
