...
2. Confirm which branch's docs you're auditing (`git branch --show-current`). Shared docs are authored on `dev`; if you're not on `dev`, read doc contents via `git show dev:<path>` rather than assuming the working tree reflects dev — do not switch branches yourself. Note that in this repo implementation code lives on the `backend`/`frontend` branches, so when checking doc-vs-implementation, read the relevant source from whichever branch actually holds it (e.g. `git show backend:<path>`, `git ls-tree`) rather than assuming it's present in the current working tree.
