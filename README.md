...
- **핵심 구현 요소:**
  ...
  - 매 턴 설명 제출마다 AI가 다른 플레이어들을 헷갈리게 하는 "교란" 코멘트를 실시간으로 추가
- **사용 / 시연 시나리오:** 참가자들이 각자 기기(웹/모바일)에서 공개방 목록 선택 또는 4자리 코드로 같은 방에 입장 → 방장이 카테고리·AI 봇 수를 설정하고 게임 시작 → 그룹 채팅 형태의 화면에서 순서대로 제시어 설명 제출(그때마다 AI 교란 코멘트 동반) → 익명 투표로 라이어 지목 → 결과 공개, 라이어로 지목되면 진짜 제시어를 맞혀 역전승 시도
...
| Day 5 | 매 턴 교란 코멘트 실시간 구현 · 라이어 역전승 로직 · 백엔드 완성도 높이기 |
...
| Firebase Auth 로그인 | 이메일/소셜 로그인, 소켓 handshake 시 ID 토큰 검증 | 필수 | ✅ (백엔드 검증만, 프론트 로그인 화면은 별도) |
| 라이어 역전승 | 투표로 지목된 라이어가 진짜 제시어를 맞히면 승리 | 필수 | ✅ |
| 유저 전적·친구 (로컬 DB) | 게임 결과를 로컬 DB(PostgreSQL+Prisma)에 영구 저장해 전적(승률 등)·친구 기능으로 노출 | 선택 | ✅ |
| 게임 내 라운드 재시작 | 하나의 게임에서 설명 라운드를 여러 번 반복 | 선택 | ⬜ 미구현 |

백엔드(`backend` 브랜치)는 위 항목 대부분이 구현·검증 완료 상태다. 프론트엔드 연동은 별개 진행 상황이며 갭은 [PLAN.md](./PLAN.md)의 "프론트-백엔드 연결 정합성" 섹션 참고.
