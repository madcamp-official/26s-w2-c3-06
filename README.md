...
|---|---|---|---|---|---|
| Socket.IO | 백엔드 자체 서버 | 방 생성/입장, 채팅, 턴 진행, 투표 등 실시간 게임 이벤트 | 이벤트별 payload | 이벤트별 broadcast | 상세 이벤트 목록은 PLAN.md |
| REST/SDK | Firebase Authentication | 로그인/회원가입, ID 토큰 발급 | 이메일/소셜 로그인 | ID 토큰 | 백엔드에서 `firebase-admin`으로 검증 |
| REST/SDK | Anthropic Claude API | 제시어 쌍 생성, 봇 턴 생성, 분탕질 코멘트 생성 | 프롬프트 | 생성 텍스트/JSON | Haiku 4.5로 시작, provider 교체 가능하게 래핑 |
| SDK | Firestore | 승패 기록 등 영구 데이터 저장 (선택 구현) | 문서 write | - | MVP 이후 스트레치 |

---

## 산출물 및 실행 방법

- **산출물 설명:** AI가 카테고리·제시어를 생성하고 봇으로 참여하며 매 턴 분탕질 코멘트를 다는 실시간 온라인 라이어게임 (Flutter 앱 + Node/Express 백엔드)
- **실행 환경:** Flutter(iOS/Android/Web) 클라이언트, Node.js 백엔드, Firebase 프로젝트(Auth+Firestore), Anthropic API 키
- **실행 방법:** 아래 "실행 방법" 참고
- **시연 영상 / 이미지:** (선택)
