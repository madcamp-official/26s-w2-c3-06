...
`dev` → `main`은 GitHub PR을 올리지 않고 **로컬에서 직접 `git merge`한 뒤 바로 push**한다(`git checkout main && git merge dev --no-ff` 방식). 병합 시 `CLAUDE.md`, `.claude/` 등 Claude 관련 파일은 `git rm --cached`로 제외하고 커밋한다.
